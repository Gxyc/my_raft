...
void process(void* argv){
    ...
    if(h == isvs){
        ...
        if(st.serv->role == leader){
            ...
            if(c.log.op == op_read){
                if(debug2 || debug_work)printf("client just read\n");
                int val = 0;
        ...
        if(s->role == leader){
            ...
            for(i = 0;i < server_num;i++){
                ...
                if(s->matchindex[i] < s->log_num - 1){
                    if(debug2 || debug_work)printf("time %d %d need append log\n",
...
int log_apply(Server* s){
    ...
    if(s->s_log[s->lastapplied + 1].op == op_write){
        s->s_log[s->lastapplied + 1].result = hash_insert(&hash_h,s->s_log[s->lastapplied + 1].key,s->s_log[s->lastapplied + 1].val);
        ...
        s->s_log[s->lastapplied + 1].result = hash_delete(&hash_h,s->s_log[s->lastapplied + 1].key,s->s_log[s->lastapplied + 1].val);
        ...
        s->s_log[s->lastapplied + 1].result = hash_updata(&hash_h,s->s_log[s->lastapplied + 1].key,s->s_log[s->lastapplied + 1].val);
