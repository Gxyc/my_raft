...
int process_cs(Server* serv,C_s* cs,C_r* cr){
    ...
    if(cs->log_num == 0){
        ...
        int k = serv->log_num;
        if(load_log_append(serv,cs->send_log,cs->log_num) == k+cs->log_num){
            if(debug1)printf("log append ok copy %d \n",serv->log_num-k);
            cr->success = copy_ok;
            serv->lastactivetime = get_ms();
        }
        else{
...
int get_cs(Server* s,C_s* cs,int goal_port){
   ...
   if(s->matchindex[goal_port - port_start] == s->log_num-1){
    ...
    if(debug1 && cs->log_num != 0){printf("func get cs\tget %d now cs prevlogindex: %d prevlogterm: %d term: %d commit: %d log_num %d\n"
...
int load_log_append(Server* s,Log* log,int num){
    ...
    for(i = 0;i < num;i++){
        if(s->log_num < load_log_maxsize){
            if(debug2)printf("append log op %d key %d value %d term %d\n",
            log[i].op,log[i].key,log[i].val,log[i].term);
...
int self_updata(void* argv){
    while(1){
        ...
        pthread_mutex_unlock(&server_state);
        usleep(leader_copy_time);
    }
    return;
}

int log_apply(Server* s,int index){
    if(debug2)printf("apply log index %d\n",index);
    if(s->s_log[index].op == op_write){
        int re = hash_insert(&hash_h,s->s_log[index].key,s->s_log[index].val);
        if(debug2)printf("apply log write key %d val %d term %d result %d\n",
        s->s_log[index].key,s->s_log[index].val,s->s_log[index].term,re);
        hash_print(&hash_h);
        return re;
    }
    if(s->s_log[index].op == op_read){
        //hash_print(&h);
        return 1;
    }
    if(s->s_log[index].op == op_delete){
        int re = hash_delete(&hash_h,s->s_log[index].key,s->s_log[index].val);
        if(debug2)printf("apply log delete key %d val %d term %d result %d\n",
        s->s_log[index].key,s->s_log[index].val,s->s_log[index].term,re);
        hash_print(&hash_h);
        return re;
    }
    if(s->s_log[index].op == op_updata){
        int re = hash_updata(&hash_h,s->s_log[index].key,s->s_log[index].val);
