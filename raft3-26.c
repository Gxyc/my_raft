...
int main(){
    ...
    pthread_create(&pid_check,NULL,(void*)leader_check,&s);
    ...
    while(1){
        ...
        if((get_ms() - s_active) > overtime){
            ...
            if(debug2 || debug3)printf("overtime\n");
            election(&s); 
        }
        else{
...
int listen_loop(void* argv){
    ...
    while(1){
        ...
        if(clnt_sock > 0){
            //if(debug_client)printf("listen one request\n");
            S_T st;
            st.serv = s;
            st.sock =  clnt_sock;
...
void process(void* argv){
    ...
    int h = -1;
    memcpy(&h,request,sizeof(h));

    if(debug_client && h!=3)printf("accept request %d \n",h);
    if(h == isvs){
        V_s vs;
        ...
        if(debug_client)printf("recv client\n");
        pthread_mutex_lock(&server_state);
        if(debug_test)test_plus();
        ...
        if(st.serv->role == leader){
            ...
            if(debug_client)printf("request log.op %d log.key %d log.val %d\n",
            c.log.op,c.log.key,c.log.val);
            if(c.log.op == op_read){
                if((debug2 || debug_work) && !debug_test)printf("client just read\n");
                int val = 0;
                if(find(st.serv,c.log.key,&val)){
                    ...
                    if(debug2 || debug_work)printf("find wrong\n");
                ...
                c.log.term = st.serv->currentterm;
                if(load_log_append(st.serv,&c.log,1,st.serv->lastincludeindex)){//??if log be delete in leader change??what should do??
                    int re_ap_c = st.serv->log_num - 1;
                    pthread_mutex_unlock(&server_state);
                    if(debug_lock)printf("p_c back lock\n");
                    if(debug2 || debug_work)printf("wait log apply...\n");
                    while(1){
                        pthread_mutex_lock(&server_state);
                        if(debug_lock)printf("p_c check get lock\n");
                        if(st.serv->role != leader){
                            if(debug2 || debug_work)printf("lose leader role log apply wrongat time %d\n",
                            get_ms()%simple_time);
                        ...
                        if(st.serv->lastapplied >= re_ap_c){
                            if(debug2 || debug_work)printf("log apply ok at time %d\n",get_ms()%simple_time);
                            reply.h = commit_ok;
                            reply.result = st.serv->s_log[re_ap_c].result;
                            pthread_mutex_unlock(&server_state);
                            if(debug_lock)printf("p_c check back lock\n");
                            break;
                        }
            ...
            if(debug_client){printf("not leader,redirect\n");}
        ...
        if(debug_client){printf("unknow request\n");}
        Reply_C r;
        r.h = unknow_request;
        char buf[16];
        ...
        write(st.sock,buf,sizeof(buf));
        close(st.sock);
        if(debug_client)printf("close unknow sock\n");
        return;
    }
    close(st.sock);
    return;
...
int process_cs(Server* serv,C_s* cs,C_r* cr){
    ...
    if(cs->prevlogindex == -1){
        ...
        if(debug1)printf("recv log index %d term %d\n",cs->prevlogindex,cs->prevlogterm);
        if(debug1){printf("log mismatch my log index %d term is %d\n",cs->prevlogindex,serv->s_log[cs->prevlogindex].term);}
        return;
    }
    
    if(cs->log_num == 0){
        if(debug2 || debug3)printf("get leader heart %d\n",get_ms()/1000%100);
        cr->success = copy_ok;
        serv->lastactivetime = get_ms();
        ...
        //int k = serv->log_num;
        if(load_log_append(serv,cs->send_log,cs->log_num,cs->prevlogindex)){
            if(debug2 || debug_work)printf("log append ok copy %d \n",serv->log_num);
            cr->success = copy_ok;
            serv->lastactivetime = get_ms();
...
int election(Server* s){
    if(s->role != leader){
        if(debug1){printf("%ld %dstart election\n",get_ms()%simple_time,port);}
        s->role = candidate;
        s->lastactivetime = get_ms();
        s->votedfor = s->id;
        s->currentterm += 1;
        ...
        if((re.ok+1) > majority){
            ...
            //s->lastactivetime = get_ms();
            leader_init(s);
            if(debug2 || debug_work){printf("%d become leader!\n",s->id);}
            for(i = 0;i < server_num;i++){
                if(node[i] == port){
                    continue;
                }
            ...
            load_log_append(s,&l,1,s->lastincludeindex);
            return;
        }
        else{
...
int leader_init(Server* s){
    s->role = leader;
    s->lastactivetime = get_ms();
    int i = 0;
    for(i = 0;i < server_num;i++){
        s->matchindex[i] = s->log_num - 1;
        s->nextindex[i] = s->log_num;
    }
    s->leaderid = empty;
    s->votedfor = s->id;
    return 0;
...
int load_log_append(Server* s,Log* log,int num,int start){
    int i = 0;
    for(i = 0;i < num;i++){
        if(s->log_num < load_log_maxsize){
            if(debug2 || debug_work)printf("append log op %d key %d value %d term %d at time%d\n",
            log[i].op,log[i].key,log[i].val,log[i].term,get_ms()%1000000);
            if(debug2)printf("append start %d\n",start);
            s->s_log[start + i + 1] = log[i];
            //s->s_log[start + i + 1].term = s->currentterm;
            //s->s_log[s->log_num].term = s->currentterm;
            s->lastincludeindex = start + i + 1;
            s->lastincludeterm = s->s_log[s->lastincludeindex].term;
            s->log_num = start + i + 2;
            if(debug2)printf("after append now log_n%d currentterm %d\n",s->log_num,s->currentterm);
            log_print(s);
        }
        else{
            return 0;
...
int self_updata(void* argv){
    while(1){
        ...
        if(s->commitindex > s->lastapplied){
            if(log_apply(s)){
                //s->lastapplied++;
                if(debug2 || debug_work)printf("log apply ok at index %d\n",s->lastapplied);
            }
            else{
                if(debug2 || debug_work)printf("apply error at index %d\n",s->lastapplied);
            }
        }
        if(s->role == leader){
            ...
            if(k > s->commitindex){
                if(debug2 || debug_work)printf("updata k %d\n",k);
                int i = 0;
                for(i = 0;i < server_num;i++){
                    if(debug2 || debug_work)printf("%d matchindex %d \t",node[i],s->matchindex[i]);
                }
                printf("\nk %d\n",k);
                s->commitindex = k;
                C_s cs;
...
int leader_check(void* argv){
    while(1){
        Server* s = (Server*)argv;
        pthread_mutex_lock(&server_state);
        //if(debug2)printf("leader_check..\t");
        if(debug_lock)printf("leader_check get lock\n");
        if(s->role == leader){
            int i = 0;
            for(i = 0;i < server_num;i++){
                if(node[i] == s->id)continue;
...
int process_cr(Server* s,C_r* cr,C_s* cs,int from_port){
    ...
    if(cr->success == cr_error_dismatch){
        if(s->matchindex[from_port - port_start] > -1){
...
int log_apply(Server* s){
    if(debug2 || debug_work)printf("start apply log index %d op%d at time%d\n",
    s->lastapplied+1,s->s_log[s->lastapplied+1].op,get_ms()%1000000);
    log_print(s);
    if(s->s_log[s->lastapplied + 1].op == op_write){
        s->s_log[s->lastapplied + 1].result = hash_insert(&hash_h,s->s_log[s->lastapplied + 1].key,s->s_log[s->lastapplied + 1].val,&mp);
        if(debug2 || debug_work)printf("apply log write key %d val %d term %d result %d\n",
        s->s_log[s->lastapplied + 1].key,s->s_log[s->lastapplied + 1].val,s->s_log[s->lastapplied + 1].term,s->s_log[s->lastapplied + 1].result);
        hash_print(&hash_h);
        //if(re == ){
        s->lastapplied++;
        return 1;
        ...
        s->s_log[s->lastapplied + 1].result = hash_delete(&hash_h,s->s_log[s->lastapplied + 1].key,s->s_log[s->lastapplied + 1].val,&mp);
        if(debug2 || debug_work)printf("apply log delete key %d val %d term %d result %d\n",
        s->s_log[s->lastapplied + 1].key,s->s_log[s->lastapplied + 1].val,s->s_log[s->lastapplied + 1].term,s->s_log[s->lastapplied + 1].result);
        hash_print(&hash_h);
        //if(re){
            s->lastapplied++;
        ...
        s->s_log[s->lastapplied + 1].result = hash_updata(&hash_h,s->s_log[s->lastapplied + 1].key,s->s_log[s->lastapplied + 1].val,&mp);
        if(debug2 || debug_work)printf("apply log updata key %d val %d term %d result %d\n",
        s->s_log[s->lastapplied + 1].key,s->s_log[s->lastapplied + 1].val,s->s_log[s->lastapplied + 1].term,s->s_log[s->lastapplied + 1].result);
        hash_print(&hash_h);
        //if(re){
            s->lastapplied++;
...
int log_print(Server* s){
    int i = 0;
    for(i = 0;i < s->log_num;i++){
        if(debug2)printf("--------log %d op %d key %d val%d term %d\n",
        i,s->s_log[i].op,s->s_log[i].key,s->s_log[i].val,s->s_log[i].term);
    }
}
...
int find(Server* s,int key,int* val){
    if(hash_find(&hash_h,key,val))return 1;
    return 0;
}
